/* Описание БД:
База данных состоит из 7 таблиц, позволяющих организовать работу системы учета электроэнергии 
и педоставлению отчетов потребления управляющим компаниям. 
Таблицы: 
- Организации - содержит id организации и ее название;
- Пользователи и подписчики - содержит id и имя пользователя и роли (будут использоваться в представлениях "отчет" 
  для пользователей с разными ролями); 
- Объекты - содержит id, адрес и его название;
- Модемы - содержит id, название и серийный номер;
- Приборы учета - содержит id, модель, название и серийный номер;
- Регуляторы - содержит id, модель, название и серийный номер;
- Данные - содержит id записи, тариф "день" Т_1, тариф "ночь" Т_2, дата, за которую считался архив с тарифами;
...
-- пример запроса с JOIN-соединением таблицы users_subs и object с зависимостью по org_id

use Electrouchet;

select distinct u.name, u.role
from users_subs as u
join objects as o
on u.org_id = o.org_id

/*Запрос на вывод оборудования объектов и кол-во хранимых архивов 
(выдает ошибку 1054 ссылаясь на отсутствие колонки modems.id. 
предыдущие два select выполняются верно и они точно такие же)*/ 

use Electrouchet;
select address,
	(select name from modems where objects.id = modems.object_id) as modem_model,
    (select serial_num from modems where objects.id = modems.object_id) as modem_serial,
    (select name from device where modems.id = device.modem_id) as device_model
  --  (select serial_num from device where modems.id = device.modem_id) as device_serial,
  --  (select name from regulator where modems.id = regulator.modem_id) as regulator_model,
   -- (select serial_num from regulator where modems.id = regulator.modem_id) as regulator_serial,
   -- (select count(*) from data where device.id = data.device_id) as data_count
from objects;

-- Представление отчета "Итоговые значения"

use Electrouchet;
drop view if exists otchet1;
create view otchet1 as
select sum(t_1), sum(t_2), date_archive 
from data where device_id = 2
group by date_archive
with rollup;

-- Представление запроса на количество пользователей и объектов организаций

use Electrouchet;
drop view if exists otchet2;
create view otchet2 as
select id, name as organization_name, 
	(select count(*) from users_subs where organization.id = users_subs.org_id AND role = 'Пользователь') as count_users, 
    (select count(*) from objects where organization.id = objects.org_id) as count_objects
from organization;
